...
const UpdateMembership = (props) => {
  ...
  const memberShipID = atob(localStorage.getItem("memberShipID")) || "";
  ...
  const fetchMembership = async (id) => {
    try {
        const response = await axios.get(`${API_BASE_URL}/membership/GetMembership/?MembershipID=${id}`, {
            headers: { 
...
const deleteFeature = async (id) => {
    try {
        const response = await axios.delete(`${API_BASE_URL}/membership/DeleteFeature/${id}/`, {
            headers: { 
                "Content-Type": "application/json",
                'Authorization': AUTH_TOKEN
            },
  ...
  const handleAddFeature = () => {
    ...
    updatedPlanData.features.push({membership:planData.id, feature: "", sort_order: "" });
  ...
  const handleFeatureChange = (index, e) => {
    const updatedPlanData = { ...planData };
    updatedPlanData.features[index][e.target.name] = e.target.value;
    setPlanData(updatedPlanData);
  };
  

  const handleRemoveFeature = async(index, id) => {
    if (id === null){
        const updatedPlanData = { ...planData };
        updatedPlanData.features = updatedPlanData.features.filter((_, i) => i !== index);
        setPlanData(updatedPlanData);    
    }else{
        handleShow()
        setDeleteID(id)
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(formValidate).length === 0) {
      setIsLoading(true)
        const response = await axios.put(`${API_BASE_URL}/membership/UpdateMembership/${memberShipID}/`, planData, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': AUTH_TOKEN
          },
        ...
        .then((response) => {
          ...
          console.log('Pricing plan updated successfully:', response.data);
    ...
    const validiteFormInputs = async(data) => {
        ...
        if (data.monthly_price == '' || data.monthly_price == null) {
        errorObj.monthly_price = "Monthly price is required";
        }

        if (data.yearly_price == '' || data.yearly_price == null) {
            errorObj.yearly_price = "Yearly price is required";
        }

        if (data.is_popular === null || data.is_popular === undefined) {
          errorObj.is_popular = "Please choose an option";
        }
...
useEffect(()=>{
    if(memberShipID || isDeleted){
        fetchMembership(memberShipID)
    }
},[memberShipID, isDeleted])
...
console.log("planData",planData)
  ...
  return (
    <div>
            ...
            <div className="row align-items-center inner-banner">
              <div className="col-md-12 col-12 text-center">
                <nav aria-label="breadcrumb" className="page-breadcrumb">
                  ...
                  <h2 className="breadcrumb-title">Update Plan</h2>
                </nav>
              </div>
            </div>
          </div>
                          ...
                          <div className="row">
                            ...
                            <div className="col-lg-4 col-md-6">
                                ...
                                <Select
                                    ...
                                    name="is_popular"
                                    options={popular}
                                    value={popular.find(option => option.value == planData?.is_popular)}
                            ...
                            <div className="col-lg-4 col-md-6">
                              ...
                              <Select
                                  ...
                                  name="user_type"
                                  options={userType}
                                  value={planData?.user_type ? userType.find(option => option.value === planData?.user_type) : null}
                        ...
                        {planData?.features.map((feature, index) => (
                            <div key={index} className="row">
                                ...
                                <div className="col-lg-1 col-md-6">
                                    ...
                                    onClick={() => handleRemoveFeature(index, feature?.id ? feature.id: null )}
                                    >
                                    Remove
                                    </button>
                                    </div>
      ...
      <MessagePopup title={"Pricing plan feature"} message={"Are you sure to delete the plan feature"} close={handleClose} show={show} yes={handleYes} no={handleNo}/>
      <SiteFooter/>
    </div>
  );
};

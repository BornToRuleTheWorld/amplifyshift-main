...
import { API_BASE_URL, AUTH_TOKEN, disciplineOptions, specialtyOptions, visitType, jobTypeOptions, paginationDefaultCount, workSettingOptions, langOptions, boolOption, ColourOption, SingleValue, MultiValueLabel } from "../../config.js";
...
const AdminContractList = (props) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/contract/GetAllContracts/`, data, {
        headers: {
      ...
      if (response?.status === 200) {
        ...
        if (response.data.data.length > 0) {
          ...
          setUserData([]);
          setSucMessage('No contract data available');
          setError(null);
        }
  ...
  const handleContractView = (id) => {
    ...
    history.push("/site-user/contract-view")
  }
          ...
          <div className="table-responsive custom-table">
            <table className="table table-center mb-0  table-striped table-hover">
              ...
              <tbody>
                ...
                {userData.length === 0 ? (
                  <tr><td colSpan="15" style={{ fontSize: "19px", padding: "15px", }}>Contracts Not Found</td></tr>
                ) : (
                  userData.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td>{user.job_contact_person}</td>
                      <td>{user.job_contact}</td>
                      <td>{user.job_pay}</td>
                      <td>{user.job_zipcode}</td>
                      <td>{user.job_license?
                            boolOption.map(bool => {
                                if (bool.value === user.job_license) {
                                    return bool.label;
                                }
                                return "N/A";
                            })
                      ...
                      <td>
                        {user.job_cpr_bls ?
                            boolOption.map(bool => {
                                if (bool.value === user.job_cpr_bls) {
                                    return bool.label;
                                }
                                return null;
                            })
                      ...
                      <td>
                        <div className="action-item">
                          ...
                          <Link to="#" onClick={()=> handleContractView(user.id)}>

...
import { doctor_thumb_02 } from "../../imagepath.jsx";
//import { Phone, PhoneOff } from "feather-icons-react/build/IconComponents";
import { Phone, PhoneOff } from "feather-icons-react/build/IconComponents/index.js";
...
import ContractMessage from "../messages/contractMessage.jsx";
import MapEmbed from "../Maps.js";

const ProfContractViewPage = (props) => {
  // Map
  ...
  const currentContractID = atob(localStorage.getItem('ContractID')) || "";
  const currentUser = atob(localStorage.getItem('userID')) || "";
  const contractFacID = atob(localStorage.getItem('contractFacID')) || "";
  ...
  const fetchContract = async(id) => {
      setError(null);
      await axios.get(`${API_BASE_URL}/contract/GetContract/?ContractID=${id}`, {
          headers: {
      ...
      .then((response) => {
        ...
        if(contractData){
          const state_name = contractData.job_state ? stateOptions.find(option => option.value == contractData.job_state).label : null
          const cntry_name = contractData.job_country ? cntryOptions.find(option => option.value == contractData.job_country).label : null
          setAddress(`${contractData.job_address1} ${contractData.job_address2} ${contractData.job_city} ${state_name} ${cntry_name} ${contractData.job_zipcode}`)
        }
      })
      .catch((err) => {
              ...
              <nav aria-label="breadcrumb" className="page-breadcrumb">
                <ol className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active">
                    <Link to="/professional/contracts">Contracts</Link>
                  </li>
                </ol>
                <h2 className="breadcrumb-title">Contract</h2>
              </nav>
            </div>
          </div>
        </div>
              ...
              <div className="row text-center" >
                ...
                <div className="col-lg-6 text-start">
                  <div className="alert alert-primary alert-dismissible fade show" role="alert">
                    <div className="row">
                      <div className="col-7">
                        <p><strong>Contract ID :</strong>&nbsp;{data.contract_no}</p>
                        <p><strong>Created On:</strong>&nbsp;{convertToUS(data.created_on,"Datetime")} </p>
                      </div>
                      <div className="col-5">
                        <p><strong>Start Date:</strong>&nbsp;{convertToUS(data.start_date,"Date")} </p>
                        <p><strong>End Date:</strong>&nbsp;{convertToUS(data.end_date,"Date")} </p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="alert alert-danger alert-dismissible fade show" role="alert">
                    <div className="row">
                      <div className="col-auto">
                        <p><strong>Professional Name :</strong>&nbsp;{data.professional_name}</p>
                        <p><strong>Status:</strong>&nbsp;{data.status}</p>
                      </div>
                      <div className="col-auto">
                        <p><strong>Payment:</strong>&nbsp;${data.job_pay}/hr</p>
                        <p><strong>Total Hours:</strong>&nbsp;{`${data.contract_hours} ${data.contract_hours <= 1 ? "Hr" : "Hrs"}`}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card doc-profile-card">
                    ...
                    <div className="doctor-widget doctor-profile-two border-bottom">
                        ...
                        <div className="doc-info-cont">
                            ...
                            <p>
                              <b>Experience required</b> - {yearOptions.map(year => {if (year.value === data.years_of_exp) {return year.label} return null })},&nbsp;
                              <b>License required</b> - {data.job_license ? boolOption.map(option => {if (option.value == data.job_license) {return option.label;}return "";}) : "N/A"},&nbsp;  <b>CPR/BLS</b> - {data.job_cpr_bls ? boolOption.map(option => {if (option.value == data.job_cpr_bls) {return option.label;}return "";}) : "N/A"},&nbsp;  <b>Pay</b> - ${data.job_pay}/hr
                            </p>
                        ...
                        <div className="row align-items-center">
                            ...
                            <div className="clinic-info">
                                ...
                                <h5>{data.job_address1}</h5>
                                <p>{(data.job_address2) ? `${data.job_address2},` : null}{stateOptions.find(option => option.value == data.job_state)?.label ?? null}, {cntryOptions.find(option => option.value == data.job_country)?.label ?? null}, {data.job_zipcode}</p>
                                </div>
                            </div>
                            </div>
                            <div className="col-12">
                ...
                <div className="appointment-patient">
                  ...
                  <div className="tab-content pt-0 mb-3">
                    <div id="work_hours" className="tab-pane fade show active">
                      ...
                      <div className='row'>
                        <div className="col offset-11 mb-1">
                          <FaCalendarAlt onClick={handleWorkHoursView} style={{fontSize:"20px"}}  className='text-success text-end'/>
                        </div>
                      </div>
                      {showWorkHours
                      ?
                      <WorkHours ID={currentContractID} Role={'Contract'} View={'AdminStyle'}/>
                      :
                      <WorkHoursCalender ID={currentContractID} Role={'Contract'} />
                      }
                    </div>
                  </div>
                </div>
...
export default ProfContractViewPage;

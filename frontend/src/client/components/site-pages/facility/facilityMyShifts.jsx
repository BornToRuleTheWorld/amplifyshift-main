...
const FacilityMyShifts = (props) => {
  ...
  const handleStatusUpdate = async (status, slotId = null) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.Status.Code === "Success") {
        ...
        setError("Failed: " + (response.data?.Result || 'Unknown error'));
      }
    } catch (error) {
  ...
  const getProfessionals =  async(id) => {
    try {
      ...
      setError("An error occurred while getting professionals");
    }
  }
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-lg-12 col-xl-12">
              {/* <div className="dashboard-header mt-3">
                <h3></h3> */}
                  ...
                  <div className="row mb-3 mt-3"> 
                    <div className="col-3">
                      ...
                      <Select
                        ...
                        value={statusFilter ? contractHourOptions.find(status => status.value === statusFilter) : null}
              ...
              <div className="tab-content appointment-tab-content">
                <div className="row mb-3">
                  <div className="offset-8 col-4 text-end">
                    <button
                      ...
                      const slotDetails = slotData.find(slot => slot.id === slotInfo.slot);
                          return (
                            <tr key={slotInfo.slot_id}>
                              ...
                              <td>{slotInfo.date}</td>
                            {(slotDetails && slotInfo.slot_type === "Slots") ?
                              <td>
                              ...
                              <td>
                               ...
                               <td>
                                {['Cancelled', 'Completed'].includes(slotInfo.status)
                                ?
                                "--"
                                :
                                <>
                                <button className="btn-sm btn-danger rounded-pill" onClick={() => handleStatusUpdate('Cancelled', slotInfo.slot_id)}>Cancel</button>&nbsp;&nbsp;
                                <button className="btn-sm btn-success rounded-pill" onClick={() => handleStatusUpdate('Completed', slotInfo.slot_id)}>Completed</button>
                                </>
                                }
                              </td>
                            </tr>
...
export default FacilityMyShifts;
